...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    await logSanityInteraction('error', `Failed to update service: ${errorMessage}`, 'service', _id ?? undefined, 'admin', false, { errorDetails: errorMessage, payload: 'FormData received' });
    return NextResponse.json({ message: errorMessage }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    await logSanityInteraction('error', `Failed to delete service: ${errorMessage}`, 'service', id ?? undefined, 'admin', false, { errorDetails: errorMessage, payload: 'Service ID: ' + id });
    return NextResponse.json({ message: errorMessage }, { status: 500 });
  }
}