    ...
    import {
      ...
      useTheme,
      Tag,
      TagLabel, // <--- Keep TagLabel if used, otherwise remove
      Divider,
      SimpleGrid,
      Image,
    } from '@chakra-ui/react';
    import NextLink from 'next/link'; // <--- Keep NextLink if used, otherwise remove
    ...
    export default function BarberDailyAppointmentsClient({ barbers, todayAppointments, upcomingAppointments }: BarberDailyAppointmentsClientProps) {
      ...
      // Color mode values - Define these at the top level of the component
      ...
      // Define specific colors for appointment cards here
      const apptCardBgLight = useColorModeValue('gray.50', 'gray.700'); // For individual appointment cards
      const apptCardBorderLight = useColorModeValue('gray.200', 'gray.600'); // For individual appointment cards
                      ...
                      <VStack align="stretch" spacing={3}>
                        {(todayAppointmentsByBarber[barber._id] && todayAppointmentsByBarber[barber._id].length > 0) ? (
                          todayAppointmentsByBarber[barber._id].map(appt => (
                            <Box key={appt._id} p={3} bg={apptCardBgLight} borderRadius="md" borderWidth="1px" borderColor={apptCardBorderLight}> {/* <--- Use defined colors */}
                        ...
                        {barbers.map(barber => {
                          ...
                          return (
                            <Box
                              ...
                              borderRadius="md"
                              borderWidth="1px"
                              borderColor={apptCardBorderLight} // <--- Use defined colors
                              bg={apptCardBgLight} // <--- Use defined colors
                            >
                              ...
                              <VStack align="stretch" spacing={2}>
                                {apptsForBarberOnDate.length > 0 ? (
                                  apptsForBarberOnDate.map(appt => (
                                    <Box key={appt._id} borderBottom="1px solid" borderColor={apptCardBorderLight} pb={2}> {/* <--- Use defined colors */}
                                      <Flex justify="space-between" align="center" mb={1}>
                                        <Text fontWeight="semibold" color={textColorPrimary}>
                                          {format(parseISO(appt.dateTime), 'hh:mm a')}
                                        </Text>
